...
from agents.travel_schedule_agent import create_travel_schedule_agent, get_mcp_servers_config
...
async def main():
    # Create global MCP client for travel_schedule_agent
    print("Initializing MCP client...")
    # Use async context manager to create and manage MCP client
    async with MultiServerMCPClient(get_mcp_servers_config()) as mcp_client:
        print("MCP client initialization complete")
        
        try:
            # Create agents
            print("Initializing Travel Agent system...")
            # Pass MCP client to travel_schedule_agent
            travel_schedule_agent = await create_travel_schedule_agent(model, mcp_client)
        ...
        # No need to explicitly close MCP client, async with handles it automatically
